...
class EditTest {
    ...
    fun sameContainerStructuralConflict() {
        ...
        val seq = document.document(0)!!.root().mappingEntry(0)!!.value()
        val transaction = EditTransactionBuilder.new(document)
            .insertSequenceElement(seq.nodeRef(), PvBoolean(true), AssociationPlacement.Start)
            .insertSequenceElement(seq.nodeRef(), PvBoolean(false), AssociationPlacement.End)
            .build()
    ...
    fun utf16EditsKeepEncoding() {
        ...
        val transaction = EditTransactionBuilder.new(document)
            ...
            .build()
        val commit = document.commit(transaction)
        assertEquals("Utf16Le", commit.document.source().encodingFacts.selected.asStr())
