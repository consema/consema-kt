...
class ProjectionTest {
    ...
    fun projectExpressionPolicy() {
        ...
        // The item stays an attribute; the VALUE member carries the
        // expression record (RFC 0014 §8.2).
        val count = projected[0] as PvObject
        assertEquals("attribute", (count.get("kind") as PvString).value)
        val countValue = count.get("value") as PvObject
        assertEquals("expression", (countValue.get("kind") as PvString).value)
        val expressionRecord = countValue.get("expression") as PvObject
        ...
        val nameValue = name.get("value") as PvObject
        val nameRecord = nameValue.get("expression") as PvObject
