...
class FormationGoldenTest {
    ...
    fun `mixed content order is preserved with all child kinds`() {
        ...
        // The final text occurrence is the 6th child (0-based index 5 was
        // the PI), so the explicit assertion is on the root text children.
    ...
    fun `utf16le source with bom is byte exact against the golden hex`() {
        ...
        // bytes of the decoded content under the source index.
        val span = text.span
        val spanBytes = document.source().rawBytes().copyOfRange(span.startByte, span.endByte)
        assertEquals("中文", String(spanBytes, Charsets.UTF_8))
    }
