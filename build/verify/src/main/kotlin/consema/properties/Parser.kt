...
private fun decodeJavaString(atoms: List<Atom>, atomIndices: List<Int>): JavaStringDecode {
    ...
    while (cursor < atomIndices.size) {
        ...
        val (kind, consumed): Pair<PropertiesEscapeKind, Int> = when (next) {
            0x75 -> {
    ...
    return JavaStringDecode.Complete(DecodedJavaString(units.toCharArray(), escapes, unicodeEscapes))
}
