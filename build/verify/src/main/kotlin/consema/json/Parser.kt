...
private fun lex(
    ...
    while (offset < bytes.size) {
        ...
        var classKind: LexemeClass = when {
            ...
            octet == 0x2f && offset + 1 < bytes.size && bytes[offset + 1] == 0x2f -> {
                offset += 2
                while (offset < bytes.size) {
                    val next = bytes[offset].toInt() and 0xff
            ...
            octet == 0x2f && offset + 1 < bytes.size && bytes[offset + 1] == 0x2a -> {
                offset += 2
                var closed = false
...
private fun decodeIdentifierEscape(bytes: ByteArray, offset: Int): Int? {
    ...
    if (bytes[offset] != 0x5c || bytes[offset + 1] != 0x75) return null
    var value = 0
    for (i in 0 until 4) {
...
private fun validJsonNumber(bytes: ByteArray, start: Int, end: Int): Boolean {
    var index = start
    if (index < end && bytes[index] == 0x2d) {
        index += 1
    }
    when {
        index < end && bytes[index] == 0x30 -> index += 1
        index < end && bytes[index] in 0x31..0x39 -> {
            index += 1
            while (index < end && bytes[index] in 0x30..0x39) {
                index += 1
            }
        }
        else -> return false
...
internal fun validJson5Number(text: String): Boolean {
    ...
    if (unsigned.startsWith("0x") || unsigned.startsWith("0X")) {
        ...
        return hex.isNotEmpty() && hex.all { it.isDigit(16) }
    }
    var index = 0
...
private class Parser(
    ...
    internal fun parseValue(depth: Int): Int {
        ...
        return when (token.kind) {
            ...
            TokenKind.Number -> {
                position += 1
                val text = source.substring(token.start, token.end)
            ...
            TokenKind.String -> {
                position += 1
                val decoded = decodeJsonString(source.substring(token.start, token.end), profile)
                if (decoded != null) {
                    if (decoded.second) {
                        sink.push(
                            sourceDiagnostic(
            ...
            TokenKind.Identifier -> {
                if (!profile.isJson5()) {
                    ...
                    position += 1
                    val text = decodeJson5Identifier(source.substring(token.start, token.end))
                    val kind = when (text) {
                ...
                if (first != null) {
                    ...
                    sink.push(
                        sourceDiagnostic(
                            ...
                            related = listOf(
                                Diagnostic.RelatedSourceLocation(
                                    "first-member",
                                    Diagnostic.SourceLocation.of(
    ...
    private fun parseObjectKey(depth: Int): Int {
        ...
        position += 1
        val name = decodeJson5Identifier(source.substring(token.start, token.end))
...
internal fun parseJson5Number(text: String): InternalValueKind {
    ...
    when (unsigned) {
        "Infinity" -> {
            val bits = if (negative) 0xfff0_0000_0000_0000L else 0x7ff0_0000_0000_0000L
            return InternalValueKind.BinaryFloat64(bits)
        }
        "NaN" -> {
            val bits = if (negative) 0xfff8_0000_0000_0000L else 0x7ff8_0000_0000_0000L
            return InternalValueKind.BinaryFloat64(bits)
        }
    }
    if (unsigned.startsWith("0x") || unsigned.startsWith("0X")) {
