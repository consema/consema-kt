...
private fun Document.prepareOperation(
    ...
    is EditOperation.InsertMember -> prepareInsertMember(operation.object, operation.name, operation.value, operation.placement)
...
private fun Document.prepareMoveMember(
    ...
    val destination = when (placement) {
        ...
        is AssociationPlacement.Before, is AssociationPlacement.After -> {
            ...
            val position = remaining.indexOf(anchor)
...
private fun validateDependencies(transaction: EditTransaction) {
    ...
    for (operation in transaction.operations) {
        ...
        when (operation) {
            is EditOperation.RemoveMember, is EditOperation.RemoveArrayElement -> {
                removed.add(operation.target)
            }
            is EditOperation.InsertMember, is EditOperation.InsertArrayElement ->
...
private fun renderNonFiniteStyle(bits: Long, style: NonFiniteLexicalStyle): ByteArray? {
    val text: String = when (bits) {
        0x7ff0_0000_0000_0000L -> if (style.explicitPlus) "+Infinity" else "Infinity"
        0xfff0_0000_0000_0000L -> "-Infinity"
        0x7ff8_0000_0000_0000L -> if (style.explicitPlus) "+NaN" else "NaN"
        0xfff8_0000_0000_0000L -> "-NaN"
        else -> return null
    }
