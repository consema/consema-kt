...
//
// Data authority:
//   - RFC 0014 §9 (docs/rfcs/0014-hcl-family-profiles-v1.md:574-628):
...
//     partial provenance.
//   - RFC 0004 §3, §7-§8 (docs/rfcs/0004-...md:56-92, 171-218): the common
//     MaterializationRequest fields and the completion algebra.
//   - crates/consema-hcl/src/materialization.rs pins the record validation
...
private class RecordValidator(
    ...
    fun validateBody(value: PortableValue, path: ValuePath): Body {
        ...
        if (record.value != HCL_BODY_RECORD) {
            // The vector pins the stable failure name "invalid-record" for
            // a wrong record identity (hcl-v1.json
            // hcl.materialization.unrepresentable sample 2).
            throw HclMaterializationException(
                HclMaterializationFailure.Unrepresentable("invalid-record"),
            )
        }
        val itemsValue = entries[1].value as? PvArray
            ?: throw HclMaterializationException(
                HclMaterializationFailure.Unrepresentable("body-record"),
            )
    ...
    private fun validateRecordValue(value: PvObject, path: ValuePath): ValueNode {
        ...
        return when (kind) {
            ...
            "object" -> {
                ...
                val entriesValue = fields[1] as? PvEntryMapping
                    ?: throw HclMaterializationException(
                        HclMaterializationFailure.Unrepresentable("object"),
                    )
                val entries = entriesValue.entries().mapIndexed { index, entry ->
