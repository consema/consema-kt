 ...
 * cross-reference; toml_edit parser/*.rs is the behavior authority). */
internal class Parser(private val src: ByteArray) {
    ...
    private fun trySpecialFloat(start: Int): PValue? {
        ...
        if (hasPrefix(src, cursor, "inf")) {
            ...
            return PValue(
                SpanRange(start, pos), PValueKind.FLOAT,
                bits = if (negative) 0xFFF0000000000000L else 0x7FF0000000000000L,
            )
        }
        if (hasPrefix(src, cursor, "nan")) {
            ...
            return PValue(
                SpanRange(start, pos), PValueKind.FLOAT,
                bits = if (negative) 0xFFF8000000000000L else 0x7FF8000000000000L,
            )
        }
        return null
    }
...
internal sealed class InternalItemKind {
    data class String(val value: String) : InternalItemKind()
    data class Integer(val value: Long) : InternalItemKind()
    data class Float(val bits: Long) : InternalItemKind()
    data class Boolean(val value: Boolean) : InternalItemKind()
