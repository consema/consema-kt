...
sealed class EditFailureKind {
    ...
    data class ResourceLimit(val name: String) : EditFailureKind()
...
fun TomlDocument.commit(transaction: EditTransaction): EditCommit {
    ...
    prepared.sortBy { it.oldSpan.startByte to it.oldSpan.endByte }
...
private fun TomlDocument.prepareDelimitedInsertion(
    ...
    if (associations.isEmpty()) {
        when (placement) {
            ...
            AssociationPlacement.Before, AssociationPlacement.After ->
...
internal fun canonicalLiteral(value: PortableValue): String = when (value) {
    ...
    is consema.core.PvDate -> {
        val year = value.year.toLongOrNull()?.toInt() ?: -1
    ...
    is consema.core.PvLocalDateTime -> {
        val date = value.date
        val time = value.time
        val year = date.year.toLongOrNull()?.toInt() ?: -1
