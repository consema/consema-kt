 ...
 * report events (the v7 registry of error_registry.rs:1339). */
 ...
 * document (materialization.rs:19-34). Failed attempts contain no Document
 ...
 * (materialization.rs:36-45): a complete scalar/array/inline-table value
...
/** Validates the frozen request contract (materialization.rs:81-99):
 ...
 * conversion (materialization.rs:116-176). */
...
/** The bounded canonical TOML writer (materialization.rs:188-547). */
internal class TomlWriter(
     ...
     * selected final newline (materialization.rs:211-259). */
    ...
    /** Writes one canonical value (materialization.rs:261-336). */
        ...
        when (value) {
            ...
            is consema.core.PvDate -> {
                val text = canonicalDateText(
                    value.year.toLongOrNull()?.toInt() ?: -1,
                    value.month,
                    value.day,
                ) ?: unrepresentable(path, value.kind)
            ...
            is consema.core.PvLocalDateTime -> {
                ...
                val text = canonicalLocalDateTimeText(
                    date.year.toLongOrNull()?.toInt() ?: -1,
                    date.month,
                    date.day,
                    time.hour,
                    time.minute,
...
/** The bounded byte accumulator (materialization.rs:569-611). */
