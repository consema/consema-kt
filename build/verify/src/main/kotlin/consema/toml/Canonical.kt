...
internal fun canonicalFloatText(bits: Long): String? {
    if (bits == 0x7FF8_0000_0000_0000L) return "nan"
    if (bits == 0xFFF8_0000_0000_0000L) return "-nan"
    if (bits == 0x7FF0_0000_0000_0000L) return "inf"
    if (bits == 0xFFF0_0000_0000_0000L) return "-inf"
    if (bits == 0x8000_0000_0000_0000L) return "-0.0"
...
internal fun canonicalLocalDateTimeText(
    ...
    "$date${canonicalTimeText(hour, minute, second, nanoseconds)}"
}
...
internal fun exactNanoseconds(fraction: PvDecimal): Long? {
    ...
    val exponent = fraction.exponent.toLongOrNull() ?: return null
    if (exponent !in -9..0) {
        return null
    }
    var nanoseconds = fraction.coefficient.toLongOrNull() ?: return null
    if (nanoseconds < 0) {
        return null
    }
