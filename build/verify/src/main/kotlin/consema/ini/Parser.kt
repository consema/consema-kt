    ...
    when (error.kind) {
        ...
        consema.document.SourceErrorKind.INVALID_SEQUENCE ->
            IniFormationException(
                ...
                "ini parse: invalid source sequence",
...
private class Parser(
    ...
    private fun addPythonContinuation(lineIndex: Int, indent: Int) {
        ...
        for (pendingIndex in pending) {
            val pendingLine = lines[pendingIndex]
            logicalLines[state.logicalIndex].physicalLines.add(
                physicalLines[pendingLine.physicalIndex].nodeRef,
            )
            joined.append('\n')
        }
        logicalLines[state.logicalIndex].physicalLines.add(physical.nodeRef)
        joined.append('\n')
    ...
    private fun pushEntrySyntax(
        ...
        pushPieceLocal(lineIndex, key.first, key.last, StructuralPieceKind.Token, IniSyntaxKind.EntryKey)
        pushPieceLocal(lineIndex, delimiter.first, delimiter.last, StructuralPieceKind.Token, IniSyntaxKind.Delimiter)
        if (quote != null) {
            pushPieceLocal(lineIndex, quote.first.first, quote.first.last, StructuralPieceKind.Token, IniSyntaxKind.Quote)
            if (value.first < value.last) {
                pushPieceLocal(lineIndex, value.first, value.last, StructuralPieceKind.Token, IniSyntaxKind.EntryValue)
            }
            pushPieceLocal(lineIndex, quote.second.first, quote.second.last, StructuralPieceKind.Token, IniSyntaxKind.Quote)
    ...
    private fun pushWindowsValueSyntax(
        ...
        pushEntrySyntax(
            ...
            quote = Pair(
                IntRange(literalStart, valueStart),
                IntRange(valueEnd, literalEnd),
            ),
        )
    }
    ...
    private fun assignDuplicateGroups() {
        ...
        val entryGroups = sortedMapOf<Pair<String, String>, MutableList<Int>>()
