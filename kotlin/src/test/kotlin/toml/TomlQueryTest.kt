...
import consema.toml.TomlSyntaxMatch
...
class TomlQueryTest {
    ...
    fun syntaxKindTextOrder() {
        val newlines = QueryExpression(consema.protocol.ExpressionKind.Input).then(
            OperatorCall("toml.syntax-kind-is", 1).withArgument("kind", PvString("Newline")),
        )
        val comment = QueryExpression(consema.protocol.ExpressionKind.Input).then(
    ...
    @Test
    fun syntaxKindBare() {
        val expression = QueryExpression(consema.protocol.ExpressionKind.Input).then(
    ...
    fun syntaxCancelled() {
        ...
        val failure = assertFailsWith<QueryFailureException> {
            executeTomlSyntaxQuery(
                syntaxExecutable(QueryExpression(consema.protocol.ExpressionKind.Input)),
                document("a=1\n"),
                TomlQueryLimits.default,
                cancelled,
            )
    ...
    fun syntaxRejectInvalidKind() {
        val failure = assertFailsWith<QueryFailureException> {
            QueryDefinition(Domains.tomlLosslessSyntaxV1())
                .withExpression(
                    QueryExpression(consema.protocol.ExpressionKind.Input).then(
                        OperatorCall("toml.syntax-kind-is", 1).withArgument("kind", PvString("newline")),
                    ),
                )
                .validate()
    ...
    fun domainMismatchIsRefused() {
        val foreign = ExecutableQuery.bind(
            QueryDefinition(Domains.jsonNativeV1())
                .withExpression(QueryExpression(consema.protocol.ExpressionKind.Input))
                .validate(),
            capabilities(),
        )
        val failure = assertFailsWith<QueryFailureException> {
