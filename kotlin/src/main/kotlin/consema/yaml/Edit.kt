...
private fun Document.mappingPlacement(
    ...
    when (placement) {
        ...
        is AssociationPlacement.Before, is AssociationPlacement.After -> {
            val anchor = (placement as? AssociationPlacement.Before)?.anchor
                ?: (placement as AssociationPlacement.After).anchor
            val (container, ordinal) = resolveMappingEntry(anchor)
...
private fun Document.sequencePlacement(
    ...
    when (placement) {
        ...
        is AssociationPlacement.Before, is AssociationPlacement.After -> {
            val anchor = (placement as? AssociationPlacement.Before)?.anchor
                ?: (placement as AssociationPlacement.After).anchor
            val (container, ordinal) = resolveSequenceItem(anchor)
            if (container != expected) {
                throw EditFailureException(EditFailure.InvalidPlacement)
            }
            if (placement is AssociationPlacement.After) ordinal + 1 else ordinal
...
private fun Document.collectionInsertionPoint(
    ...
    if (collectionIsFlow(container, flowStart)) {
        ...
        return if (span != null) {
            if (spans.getOrNull(ordinal) != null) {
                span.startByte
            } else {
                span.endByte
            }
        } else {
            // Empty flow collection: insert directly in front of the flow
            // end token.
...
private class ValidationModel(
    ...
    private fun compareEdge(
        ...
        val expectedAlias = expected.alias
        val actualAlias = actual.alias
        when {
            expectedAlias == null && actualAlias == null -> {}
            expectedAlias != null && actualAlias != null &&
                expectedAlias.name == actualAlias.name -> {
                val expectedSource = expectedAlias.sourceAlias
                val actualSource = actualAlias.sourceAlias
                if (expectedSource != null && actualSource != null) {
                    state.output.aliases[expectedSource] = actualSource
                }
            }
            else -> throw EditFailureException(EditFailure.NewDocumentFormationFailed)
        }
