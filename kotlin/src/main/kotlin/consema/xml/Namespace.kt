...
//   - https://github.com/consema/consema-rs/blob/main/consema-xml/src/namespace.rs (the two frozen URIs),
//     namespace.rs (QName, ExpandedName), namespace.rs
//     (Binding, NamespaceError), namespace.rs (NamespaceScope
...
// one element is never shared mutable state (namespace.rs).
...
sealed class NamespaceError {
    ...
    /** The stable diagnostic code of this failure (parser.rs). */
    fun code(): String =
        when (this) {
...
/**
 * Immutable, ancestry-derived namespace scope (namespace.rs). A scope
...
class NamespaceScope private constructor(private val bindings: List<Binding>) {
     ...
     * (namespace.rs). The `xmlns` prefix can never be declared, the
