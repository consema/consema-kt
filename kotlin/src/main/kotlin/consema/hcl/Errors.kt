...
//       parse family: lexer.rs (byte-order-mark@1, lone-cr@1,
...
//         heredoc-marker@1), parser.rs (item@1, attribute@1, block@1,
//         label@1, expression@1, directive@1, newline@1, separator@1,
//         duplicate-attribute@1), lib.rs (encoding@1), parser.rs
//         (internal@1), parser.rs (coverage@1), parser.rs
//         (coordinates@1);
//       tfvars family: document.rs (hcl.tfvars.block-not-allowed@1);
//       limit family: parser.rs (offset-overflow@1), parser.rs
...
/** `hcl.parse.unterminated-interpolation@1` (lexer.rs): an interpolation
...
/** `hcl.parse.unterminated-directive@1` (lexer.rs): a directive without
 * its closing brace. */
const val HCL_PARSE_UNTERMINATED_DIRECTIVE = "hcl.parse.unterminated-directive@1"

/** `hcl.parse.unterminated-heredoc@1` (lexer.rs): a heredoc without its
...
/** `hcl.parse.heredoc-marker@1` (lexer.rs): a heredoc introducer without
...
/** `hcl.parse.item@1` (parser.rs): a body item failed to parse. */
const val HCL_PARSE_ITEM = "hcl.parse.item@1"

/** `hcl.parse.attribute@1` (parser.rs): an attribute failed to parse. */
const val HCL_PARSE_ATTRIBUTE = "hcl.parse.attribute@1"

/** `hcl.parse.block@1` (parser.rs): a block failed to parse (for example
 * an unterminated block body, hcl-v1.json hcl.query.error-regions). */
const val HCL_PARSE_BLOCK = "hcl.parse.block@1"

/** `hcl.parse.label@1` (parser.rs): a block label failed to parse. */
const val HCL_PARSE_LABEL = "hcl.parse.label@1"

/** `hcl.parse.expression@1` (parser.rs): an expression failed to parse;
...
/** `hcl.parse.directive@1` (parser.rs): a template directive failed to
 * parse. */
const val HCL_PARSE_DIRECTIVE = "hcl.parse.directive@1"

/** `hcl.parse.newline@1` (parser.rs): a required newline terminator was
...
/** `hcl.parse.separator@1` (parser.rs): a required separator was missing. */
const val HCL_PARSE_SEPARATOR = "hcl.parse.separator@1"

/** `hcl.parse.duplicate-attribute@1` (parser.rs): a second attribute with
...
/** `hcl.parse.encoding@1` (lib.rs): the caller-selected encoding
...
/** `hcl.parse.internal@1` (parser.rs): an internal invariant failure. */
const val HCL_PARSE_INTERNAL = "hcl.parse.internal@1"

/** `hcl.parse.coverage@1` (parser.rs): exhaustive source coverage could
 * not be constructed; fatal (RFC 0014 §3). */
const val HCL_PARSE_COVERAGE = "hcl.parse.coverage@1"

/** `hcl.parse.coordinates@1` (parser.rs): impossible source
 * coordinates; fatal (RFC 0014 §3). */
const val HCL_PARSE_COORDINATES = "hcl.parse.coordinates@1"

/** `hcl.tfvars.block-not-allowed@1` (document.rs): a block at the top
...
/** `hcl.limit.heredoc-lines@1` (lexer.rs): heredoc line count exceeded;
 * fatal. */
const val HCL_LIMIT_HEREDOC_LINES = "hcl.limit.heredoc-lines@1"

/** `hcl.limit.heredoc-bytes@1` (lexer.rs): heredoc byte count exceeded;
 * fatal. */
const val HCL_LIMIT_HEREDOC_BYTES = "hcl.limit.heredoc-bytes@1"

/** `hcl.limit.token-count@1` (lexer.rs): token count exceeded; fatal. */
const val HCL_LIMIT_TOKEN_COUNT = "hcl.limit.token-count@1"

/** `hcl.limit.syntax-pieces@1` (lexer.rs): lossless syntax piece count
 * exceeded; fatal. */
const val HCL_LIMIT_SYNTAX_PIECES = "hcl.limit.syntax-pieces@1"

/** `hcl.projection.incomplete-document@1` (projection.rs): a Recovered
 * document never projects (RFC 0014 §8.2). */
const val HCL_PROJECTION_INCOMPLETE_DOCUMENT = "hcl.projection.incomplete-document@1"

/** `hcl.projection.non-literal-expression@1` (projection.rs): a derived
...
/** `hcl.projection.unrepresentable@1` (projection.rs): a native fact
...
/** `hcl.projection.resource-limit@1` (projection.rs): a configured
...
/** `hcl.projection.core-invariant@1` (projection.rs): an internal
 * invariant failure of the record codec. */
const val HCL_PROJECTION_CORE_INVARIANT = "hcl.projection.core-invariant@1"

/** `hcl.materialization.unrepresentable@1` (materialization.rs): a
...
/** `hcl.materialization.resource-limit@1` (materialization.rs): a
...
/** `hcl.edit.duplicate-attribute@1` (edit.rs): an insertion would create
 * a duplicate attribute in one body. */
const val HCL_EDIT_DUPLICATE_ATTRIBUTE = "hcl.edit.duplicate-attribute@1"

/** `hcl.edit.block-in-tfvars@1` (edit.rs): a block operation under the
...
/** `hcl.edit.unrepresentable@1` (edit.rs): an edit value cannot be
...
/** `hcl.query.type-mismatch@1` (query.rs): a typed literal accessor
...
/** `hcl.query.non-literal@1` (query.rs): a typed literal accessor found
 ...
 * (consema-document lib.rs; the toml family precedent
 * kotlin/src/main/kotlin/consema/toml/Errors.kt). */
 ...
 * bound (consema-document lib.rs; RFC 0016 §5.1). */
...
data class HclDiagnostic(
     ...
     * of the protocol layer applies, kotlin/src/main/kotlin/consema/protocol/Diagnostic.kt).
     */
 ...
 * consema-hcl/src/lib.rs precedent). The [name] spellings are the
