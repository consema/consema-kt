...
# lib\kotlin-compiler.jar, debug\examples\emit_protocol_exchange.exe) —
...
if ($OutDir -eq '') {
    $OutDir = Join-Path $targetDir "kotlin-differential-exchange-$nonce"
}
...
$ktOutDir = Join-Path $targetDir "kotlin-differential-exchange-kt-$nonce"
...
$workDir = Join-Path $targetDir "kotlin-verify-exchange-$nonce"
...
if (-not $summary.Success) {
    $rustSkip = [regex]::Match($output, '(?m)^\[SKIP\][^\r\n]*CONSEMA_EXCHANGE_RUST_DIR is not set[^\r\n]*')
    $ktSkip = [regex]::Match($output, '(?m)^\[SKIP\][^\r\n]*CONSEMA_EXCHANGE_(KT|KOTLIN)_DIR is not set[^\r\n]*')
    if ($rustSkip.Success) {
        Write-Error "the exchange test skipped: $($rustSkip.Value.Trim())"
    } elseif ($ktSkip.Success) {
        Write-Error "the Kotlin encoder emitter skipped: $($ktSkip.Value.Trim())"
    } else {
        Write-Error "the Kotlin exchange tests did not run and printed no documented skip marker (exit $testCode)"
    }
