...
# (milestone 0.15.0 G1.5; docs/five-language-ci-design.md §3.3; the Go
...
if ($caseCount -lt 104) {
    Write-Error "normalized differential case file has $caseCount cases, want >= 104"
    exit 1
}
...
if ($OutDir -eq '') {
    $OutDir = Join-Path $targetDir 'kotlin-differential-normalized'
}
...
$ktEvidenceDir = Join-Path $targetDir 'kotlin-differential-normalized-kt'
...
$workDir = Join-Path $targetDir 'kotlin-verify-normalized'
