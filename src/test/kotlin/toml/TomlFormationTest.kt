...
class TomlFormationTest {
    ...
    fun arrayAotDistinct() {
        val document = parse(APPLICATION_TOML, TomlProfile.TOML_1_0_V1, ParseLimits.default)
        val entries = document.root().tableEntries()!!
        val database = entries.first { it.name() == "database" }.item()
        val timeouts = database.tableEntries()!!.first { it.name() == "timeouts" }.item()
    ...
    fun orderAndSpansAreExact() {
        ...
        assertEquals(16, entries[1].span.endByte)
        val y = entries[1].item()
        assertEquals(10, y.span.startByte)
        assertEquals(16, y.span.endByte)
    ...
    fun invalidUtf8FailsFormation() {
        val failure = assertFailsWith<TomlFormationException> {
            parse(byteArrayOf(0x78, 0x20, 0x3D, 0x20, 0xC3.toByte(), 0x28), TomlProfile.TOML_1_0_V1, ParseLimits.default)
        }
